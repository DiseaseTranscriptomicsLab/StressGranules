...
# Load data
OGtable = pd.read_table("SpinSim/SG2017_WC1.txt", header=0)  # Expression table (TPM values)
WeightTable = pd.read_table("SpinSim/MolecularWeights.csv", header=0, sep=" ")  # Molecular weights
...
def spinner(weight_table):
    weight_table['d'] = (((weight_table.iloc[:, 0]) ** (1/3)) * Acceleration * Time) / TubeSize * 1e-13
    return weight_table
...
if __name__ == "__main__":
    ...
    # Save result
    output_path = "./SimulatedSamples/SG2017/SG2017_MimiSEQ_SG1.csv"
